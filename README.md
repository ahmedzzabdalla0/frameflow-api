...
A production-ready **NestJS 12 + PostgreSQL + Prisma 7** backend for the FrameFlow media player. Scans a local video library, manages metadata and categories, generates thumbnails via `ffmpeg`, and streams video files — all via an open REST API with interactive **Swagger/OpenAPI** documentation.
...
- **JWT-based authentication** (via `@nestjs/passport` + `@nestjs/jwt`) is wired up but all endpoints are currently open (`@Public()`). The auth layer remains available if role-based access control needs to be re-enabled in the future.
...
- **Swagger / OpenAPI docs** (`@nestjs/swagger`) generated directly from controller and DTO decorators (`@ApiTags`, `@ApiOperation`, `@ApiResponse`, `@ApiProperty`) and served as an interactive UI at `/api/docs`.
...
- `JwtAuthGuard` and `RolesGuard` are registered globally in `AppModule`. All routes are currently decorated with `@Public()` and therefore open; remove `@Public()` and add `@Roles('ADMIN')` on any route to re-enable role-based access control.
...
Once the server is running, interactive Swagger/OpenAPI documentation is available at **`/api/docs`** — every endpoint, DTO, and response shape is documented there.

To pick up existing video files, drop them into `UPLOADS_DIR` and call `POST /api/videos/scan`, or point `UPLOADS_DIR` at your existing media folder — no file moves required, only a scan.
...
All routes below are prefixed with `/api` except `GET /video/:filename`. All routes are public (no authentication required).
...
| PUT    | `/videos/bulk-categories`  | Public | `BulkUpdateCategoriesDto { ids: number[], categories: string[] }`                                                                  | `{"ok":true,"updated":3}`                                                                                                                                                                        |
| PUT    | `/videos/:id`              | Public | `UpdateVideoDto { title?, categories?, rating? }`                                                                                  | `{"ok":true,"video":{"id":1,"title":"New title", "...":"..."}}`                                                                                                                                  |
...
| DELETE | `/videos/:id`              | Public | —                                                                                                                                  | `{"ok":true}`                                                                                                                                                                                    |
| POST   | `/videos/scan`             | Public | —                                                                                                                                  | `{"ok":true,"added":3}`                                                                                                                                                                          |
| POST   | `/videos/refresh-metadata` | Public | `RefreshMetadataDto { ids?: number[] }`                                                                                            | `{"ok":true,"refreshed":42}`                                                                                                                                                                     |
...
| POST   | `/categories/reorder`      | Public | `ReorderCategoriesDto { ids: number[] }`                                                                                           | `{"ok":true,"ids":[3,1,2,4]}`                                                                                                                                                                    |
| POST   | `/categories`              | Public | `CreateCategoryDto { name, color? }`                                                                                               | `{"ok":true,"category":{"id":5,"name":"music","color":"#e44"}}`                                                                                                                                  |
| PUT    | `/categories/:id`          | Public | `UpdateCategoryDto { name?, color? }`                                                                                              | `{"ok":true,"category":{"id":5,"name":"music-videos","color":"#00aacc"}}`                                                                                                                        |
| DELETE | `/categories/:id`          | Public | —                                                                                                                                  | `{"ok":true}`                                                                                                                                                                                    |
...
| POST   | `/settings`                | Public | `UpdateSettingsDto { default_included_categories?, default_excluded_categories?, default_pure_only?, default_intersection_only? }` | `{"ok":true}`                                                                                                                                                                                    |
| GET    | `/thumb/:relPath`          | Public | —                                                                                                                                  | binary JPEG (or a placeholder SVG if generation fails)                                                                                                                                           |
| POST   | `/set-thumb-seek`          | Public | `SetThumbSeekDto { video, seek }` (`seek` as `HH:MM:SS[.ms]`)                                                                      | `{"ok":true}`                                                                                                                                                                                    |
| POST   | `/clear-thumbs`            | Public | —                                                                                                                                  | `{"ok":true,"deleted":17}`                                                                                                                                                                       |
